...
import { SIZE } from '@/Views/GameView/components/Board/Cell/constants'
...
export function SingleGamePlayingHeader() {
  const { mark } = useSingleGameStore()
    ...
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', gap: 20, paddingHorizontal: 20 }}>
      <View style={{ width: '100%', justifyContent: 'space-between', flexDirection: 'row' }}>
        ...
        <View style={{ flex: 1, gap: 16, alignItems: 'flex-end' }}>
          <View style={{ flexDirection: 'row', gap: 8, alignItems: 'flex-end' }}>
            <Avatar size={SIZE / 3} avatar={'default'} />
            <ThemedText type="subtitle">Computer</ThemedText>
          </View>
        </View>
      </View>

      <View style={{ width: '100%', justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row' }}>
        <PlayerIndicator playerId={Player.USER} mark={mark as Mark} />
        <VSText />
        <PlayerIndicator playerId={Player.COMPUTER} mark={oppositeMark} />
      </View>
    </View>
  )
}
...
function PlayerIndicator({ mark, playerId }: { mark: Mark; playerId: string }) {
  const { activePlayer } = useSingleGameStore()
  ...
  return (
    <Animated.View
      style={[
        styles.container,
        {
          backgroundColor: PrimaryColors[theme][(mark ?? 'x') as Mark].shadow,
          borderColor: PrimaryColors[theme][(mark ?? 'x') as Mark].primary,
        },
        animatedStyle,
      ]}
    >
